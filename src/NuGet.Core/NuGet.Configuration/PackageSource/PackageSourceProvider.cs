...
namespace NuGet.Configuration
{
    public class PackageSourceProvider : IPackageSourceProvider
    {
        ...
        private Dictionary<string, IndexedPackageSource> LoadPackageSourceLookup(bool byName)
        {
            ...
            var disabledSources = new HashSet<string>(disabledSourcesSettings.GroupBy(setting => setting.Key).Select(group => group.First().Key));
            var packageSourceLookup = new Dictionary<string, IndexedPackageSource>(StringComparer.OrdinalIgnoreCase);
        ...
        private PackageSourceCredential ReadCredential(string sourceName)
        {
            ...
            if (credentialsItem != null && !credentialsItem.IsEmpty())
            {
                ...
                return new PackageSourceCredential(sourceName, username, password, credentialsItem.IsPasswordClearText);
            }
        ...
        {
            Settings.SetItemInSection(ConfigurationConstants.DisabledPackageSources, new AddItem(name, "true"), isBatchOperation: true);
        ...
        {
            var packageSourcesSection = Settings.GetSection(ConfigurationConstants.PackageSources);
            var sourceToUpdate = packageSourcesSection?.Children.Select(s => s as SourceItem)
                .Where(s => s != null && string.Equals(s.Key, source.Name, StringComparison.OrdinalIgnoreCase))?.Last();
        ...
        {
            var shouldUpdateSourceItem = !newSource.IsMachineWide && newSource.IsPersistable;
            ...
            if (string.Equals(newSource.Name, existingSource.Name, StringComparison.OrdinalIgnoreCase))
            {
                if ((!string.Equals(newSource.Source, existingSource.Source, StringComparison.OrdinalIgnoreCase) ||
                    newSource.ProtocolVersion != existingSource.ProtocolVersion) && shouldUpdateSourceItem)
                {
                ...
                if (updateEnabled)
                {
                    if (newSource.IsEnabled && existingDisabledSourceItem != null && shouldUpdateSourceItem)
                    {
                ...
                {
                    if (existingCredentialsItem != null && shouldUpdateSourceItem)
                    {
        ...
        {
            if (!source.IsMachineWide && source.IsPersistable)
            {
        ...
        public void SavePackageSources(IEnumerable<PackageSource> sources)
        {
            ...
            var existingSettingsLookup = existingSettings.ToDictionary(setting => setting.Key, StringComparer.OrdinalIgnoreCase);
            ...
            {
                var existingSourceIsEnabled = existingDisabledSourcesLookup.TryGetValue(source.Name, out var existingDisabledSourceItem);
            ...
            {
                if (existingDisabledSourcesLookup.TryGetValue(sourceItem.Value.Key, out var existingDisabledSourceItem))
                {
                    existingDisabledSourceItem.RemoveFromCollection(isBatchOperation: true);
                }

                if (existingsourceCredentialsLookup.TryGetValue(sourceItem.Value.Key, out var existingSourceCredentialItem))
                {
        ...
        public void DisablePackageSource(PackageSource source)
        {
            ...
            if(!Settings.SetItemInSection(ConfigurationConstants.DisabledPackageSources, new AddItem(source.Name, "true")))
            {
                throw new InvalidOperationException(Resources.Error_CouldNotDisableSources);
            }
        }
        ...
        public void SaveActivePackageSource(PackageSource source)
        {
            try
            {
                ...
                Settings.CreateSection(new SettingsSection(
